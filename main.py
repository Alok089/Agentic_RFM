...
# This version is more robust and relies on the raw scores.
def get_segment(r_score, f_score, m_score):
    if r_score >= 4 and f_score >= 4 and m_score >= 4:
        ...
        return 'new'
    elif ((r_score*0.2) + (f_score*0.3) + (m_score*0.5)) >= 3:
        return 'High Potential'
    elif ((r_score*0.2) + (f_score*0.3) + (m_score*0.5)) < 3 and ((r_score*0.2) + (f_score*0.3) + (m_score*0.5)) > 2:
        return 'Mid Potential'
    elif ((r_score*0.2) + (f_score*0.3) + (m_score*0.5)) <= 2:
...
def rfm_calculator():
    ...
    return render_template('index.html', customers=customers_list, message=message, error=error,
                           RFM_summary=RFM_summary, chart_data=chart_data)
